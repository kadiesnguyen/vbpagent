...
VBPClaw abstracts LLM communication behind a single `Provider` interface, allowing the agent loop to work with any backend without knowing the wire format. Six concrete implementations exist: Anthropic (native HTTP+SSE), OpenAI-compatible (covering 10+ API endpoints), Claude CLI (local binary), Codex (OAuth-based), ACP (subagent orchestration), and DashScope (Alibaba Qwen with thinking).
...
The ACP provider enables VBPClaw to orchestrate external coding agents (Claude Code, Codex CLI, Gemini CLI, or any ACP-compatible agent) as subprocesses via JSON-RPC 2.0 over stdio. This allows delegating complex code generation tasks to specialized agents while maintaining VBPClaw's unified interface.
...
1. Extract system prompt + user message from VBPClaw `ChatRequest.Messages`
...
The Claude CLI provider enables VBPClaw to delegate requests to a local `claude` CLI binary. The CLI manages session history, context files, and tool execution independently; VBPClaw only passes messages and streams responses back.
...
Claude CLI executes tools natively (filesystem, exec, web, memory). VBPClaw forwards tool results back and lets the CLI loop continue. This differs from standard providers which return tool calls for the agent loop to execute.
...
VBPClaw persists this on assistant messages and passes it back in subsequent requests. Codex performance depends on this field being echoed correctly.
