...
VBPClaw is an **AI agent gateway** — it handles agents, chat, sessions, tools, MCP servers, and memory. It supports two deployment modes:

1. **Personal / Single-tenant** — Use VBPClaw directly as your AI backend. Built-in dashboard included.
2. **SaaS / Multi-tenant** — Integrate VBPClaw behind your application. API keys bridge the two systems.
...
Use VBPClaw as a standalone AI backend with its built-in web dashboard. No separate frontend or backend needed.

```mermaid
graph LR
    U[You] -->|browser| GC[VBPClaw Dashboard<br/>+ Gateway]
...
```bash
# 1. Build and onboard
go build -o vbpclaw . && ./vbpclaw onboard
...
Integrate VBPClaw as the AI engine behind your SaaS application. Your app handles auth, billing, and UI. VBPClaw handles AI. Each tenant is fully isolated — agents, sessions, memory, teams, providers, and files.
...
**How it works:**
- Each tenant's **backend connects via a tenant-bound API key** — VBPClaw auto-scopes all data
...
sequenceDiagram
    ...
    GC-->>Admin: {key: "vbpclaw_sk_abc123..."}
...
VBPClaw determines the tenant from the credentials used to connect:

| Credential | Tenant Resolution | Use Case |
|------------|-------------------|----------|
| **Gateway token** + owner user ID | All tenants (cross-tenant) | System administration |
| **Gateway token** + non-owner user ID | Master tenant by default, or a membership-validated `X-VBPClaw-Tenant-Id` / `tenant_id` hint | Dashboard users |
| **API key** (tenant-bound) | Auto from key's `tenant_id` | Normal SaaS integration |
| **API key** (system-level) + `X-VBPClaw-Tenant-Id` | Header value (UUID or slug), while keeping the key's original role | Cross-tenant tools |
...
**Owner IDs:** Configured via `VBPCLAW_OWNER_IDS` env var (comma-separated). Only owners get cross-tenant access with the gateway token. Default: `system`.
...
| `X-VBPClaw-User-Id` | Yes | Your app's user ID (max 255 chars). Scopes sessions and per-user data |
| `X-VBPClaw-Tenant-Id` | No | Tenant UUID or slug. Only needed for system-level keys |
| `X-VBPClaw-Agent-Id` | No | Target agent ID (alternative to `model` field) |
| `Accept-Language` | No | Locale for error messages: `en`, `vi`, `zh` |

### Chat (OpenAI-Compatible)

```bash
curl -X POST https://vbpclaw.example.com/v1/chat/completions \
  -H "Authorization: Bearer vbpclaw_sk_abc123..." \
  -H "X-VBPClaw-User-Id: user-456" \
...
```bash
# List agents
curl https://vbpclaw.example.com/v1/agents \
  -H "Authorization: Bearer vbpclaw_sk_abc123..." \
  -H "X-VBPClaw-User-Id: user-456"

# List sessions
curl https://vbpclaw.example.com/v1/sessions \
  -H "Authorization: Bearer vbpclaw_sk_abc123..." \
  -H "X-VBPClaw-User-Id: user-456"
```
...
curl https://vbpclaw.example.com/v1/agents \
  ...
  -H "X-VBPClaw-Tenant-Id: acme" \
  -H "X-VBPClaw-User-Id: system"
```
...
sequenceDiagram
    ...
    participant GC as VBPClaw Gateway

    FE->>BE: User sends message (authenticated)
    BE->>GC: WS connect {token: "vbpclaw_sk_abc...", user_id: "user-456"}
...
Chat channels (Telegram, Discord, Zalo, Slack, WhatsApp, Feishu) connect **directly** to VBPClaw — no API key needed. Tenant isolation is baked into the channel instance at registration time.
...
| Concern | How VBPClaw Handles It |
|---------|-----------------------|
...
|----------|---------|-------------|
| `VBPCLAW_OWNER_IDS` | `system` | Comma-separated user IDs with cross-tenant access |
| `VBPCLAW_LOG_LEVEL` | `info` | Log level: `debug`, `info`, `warn`, `error` |
| `VBPCLAW_CONFIG` | `config.json5` | Path to gateway config file |
