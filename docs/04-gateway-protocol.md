...
The gateway is the central component of VBPClaw, serving both WebSocket RPC (Protocol v3) and HTTP REST API on a single port. It handles authentication, role-based access control, rate limiting, and method dispatch for all client interactions.
...
- `X-VBPClaw-User-Id`: required for per-user scoping
- `X-VBPClaw-Agent-Id`: specify target agent for the request
...
Agent resolution priority: `model` field with `vbpclaw:` or `agent:` prefix, then `X-VBPClaw-Agent-Id` header, then `"default"`.
...
All CRUD endpoints require `Authorization: Bearer <token>` and `X-VBPClaw-User-Id` header for per-user scoping.
