...
func promptPostgresFields() (string, error) {
	...
	dbName, err := promptString("Database name", "", "vbpclaw")
	if err != nil {
		return "", err
	}
...
func onboardWriteEnvFile(path, postgresDSN, gatewayToken, encryptionKey string) {
	var lines []string
	lines = append(lines, "# VBPClaw — auto-generated by onboard")
	lines = append(lines, "# Keep this file secret! Add to .gitignore.")
	lines = append(lines, "")
	lines = append(lines, fmt.Sprintf("export VBPCLAW_POSTGRES_DSN=%s", postgresDSN))
	lines = append(lines, fmt.Sprintf("export VBPCLAW_GATEWAY_TOKEN=%s", gatewayToken))
	lines = append(lines, fmt.Sprintf("export VBPCLAW_ENCRYPTION_KEY=%s", encryptionKey))
	lines = append(lines, "")
