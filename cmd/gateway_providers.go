...
func registerProviders(registry *providers.Registry, cfg *config.Config) {
	...
	if cfg.Providers.ClaudeCLI.CLIPath != "" {
		...
		// Build per-session MCP config: external MCP servers + VBPClaw bridge
		...
		// Enable VBPClaw security hooks (shell deny patterns, path restrictions)
...
// gatewayAddr is used to inject VBPClaw MCP bridge for Claude CLI providers.
