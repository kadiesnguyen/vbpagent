...
func runUpgradeStatus() error {
	...
	if cfg.Database.PostgresDSN == "" {
		fmt.Println("  Database:        NOT CONFIGURED (set VBPCLAW_POSTGRES_DSN)")
		return nil
	}
	...
	if s.NeedsMigration {
		fmt.Println()
		fmt.Println("  Run 'vbpclaw upgrade' to apply all pending changes.")
	}
...
func runUpgrade(dryRun bool) error {
	...
	if cfg.Database.PostgresDSN == "" {
		fmt.Println("Database not configured. Set VBPCLAW_POSTGRES_DSN to enable migrations.")
		return nil
	}
...
// If VBPCLAW_AUTO_UPGRADE=true and schema is outdated, it runs the upgrade inline.
func checkSchemaOrAutoUpgrade(dsn string) error {
	...
	if os.Getenv("VBPCLAW_AUTO_UPGRADE") == "true" {
