...
func main() {
	...
	msgID, err := protocol.SendMessage(ctx, sess, groupID, protocol.ThreadTypeGroup,
		fmt.Sprintf("VBPClaw Go E2E test - %s", time.Now().Format(time.RFC3339)))
	if err != nil {
...
func createTestImage() string {
	...
	f, err := os.CreateTemp("", "vbpclaw_e2e_*.png")
	if err != nil {
		log.Fatal(err)
	}
	f.Write(png)
...
func createTestFile() string {
	content := fmt.Sprintf("VBPClaw E2E Test File\nTimestamp: %s\nThis file tests the file upload pipeline.\n", time.Now().Format(time.RFC3339))
	f, err := os.CreateTemp("", "vbpclaw_e2e_*.txt")
	if err != nil {
		log.Fatal(err)
	}
