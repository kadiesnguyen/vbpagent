...
- VBPClaw uses directory name as the DB slug — `name` is also used as display name fallback
...
VBPClaw reads only `name` and `description` from frontmatter (the `Metadata` struct).
