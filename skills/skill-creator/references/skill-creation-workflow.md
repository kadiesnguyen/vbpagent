...
```bash
scripts/init_skill.py <skill-name> --path ~/.vbpclaw/skills-store/
```

Creates: `~/.vbpclaw/skills-store/<skill-name>/SKILL.md` template, `scripts/`, `references/`, `assets/` with examples.
...
## Step 8: Publish to VBPClaw

Register the skill in the VBPClaw database:
...
Creates: `skill-name.zip` for uploading to other VBPClaw instances via the admin UI.
...
5. Re-publish: `publish_skill(path: "~/.vbpclaw/skills-store/<name>")` — DB version auto-increments only if content changed
