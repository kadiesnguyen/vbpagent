...
## Distribution Models in VBPClaw

### 1. Publish to Current Instance (Primary)

Register skill directly in the running VBPClaw instance:

```
publish_skill(path: "~/.vbpclaw/skills-store/<name>")
```

- Copies files to managed store: `~/.vbpclaw/skills-store/<slug>/<version>/  (Docker: /app/.vbpclaw/skills-store/)`
...
Package skill as ZIP, then upload via VBPClaw admin dashboard (`/skills` page):

```bash
scripts/package_skill.py ~/.vbpclaw/skills-store/<name>
# → creates <name>.zip
```

Upload at: **Admin UI → Skills → Upload skill**

Use case: distributing skills to other VBPClaw instances without direct filesystem access.
...
Use case: ship default skills with every VBPClaw deployment.

## Version Management

VBPClaw manages versions automatically via content hash:
...
Do NOT manually set `version` in SKILL.md frontmatter — it has no effect on VBPClaw's versioning.

## Dependency Handling

After publishing, VBPClaw scans for missing Python/Node deps automatically.
...
1. Package: `scripts/package_skill.py ~/.vbpclaw/skills-store/<name>` → ZIP file
2. Share the ZIP — recipient uploads via Admin UI → Skills → Upload
3. Or contribute to `skills/` directory in the VBPClaw repo for bundling
