...
<p align="center">
  <a href="https://docs.vbpclaw.sh">文档</a> •
  <a href="https://docs.vbpclaw.sh/#quick-start">快速开始</a> •
...
**VBPClaw** 是一个多智能体 AI 网关，将大语言模型连接到你的工具、渠道和数据 —— 以单个 Go 二进制文件部署，零运行时依赖。它跨 20 多个大语言模型提供商编排智能体团队和跨智能体委托，并提供完整的多租户隔离。
...
| 功能特性                   | OpenClaw                             | ZeroClaw                                     | PicoClaw                              | **VBPClaw**                     |
| -------------------------- | ------------------------------------ | -------------------------------------------- | ------------------------------------- | ------------------------------ |
| 多租户（PostgreSQL）       | —                                    | —                                            | —                                     | ✅                             |
...
# 在 .env 中至少添加一个 VBPCLAW_*_API_KEY，然后：
make up
...
当设置了 `VBPCLAW_*_API_KEY` 环境变量时，网关会自动完成初始化，无需交互提示 —— 自动检测提供商、运行数据库迁移并填充默认数据。

> 有关构建变体（OTel、Tailscale、Redis）、Docker 镜像标签和 compose 覆盖文件，请参阅[部署指南](https://docs.vbpclaw.sh/#deploy-docker-compose)。

## 多智能体编排

VBPClaw 支持智能体团队和跨智能体委托 —— 每个智能体以其自身的身份、工具、大语言模型提供商和上下文文件运行。
...
> 有关委托详情、权限链接和并发控制，请参阅[智能体团队文档](https://docs.vbpclaw.sh/#teams-what-are-teams)。
...
完整文档请访问 **[docs.vbpclaw.sh](https://docs.vbpclaw.sh)** —— 或在 [`vbpclaw-docs/`](https://github.com/nextlevelbuilder/vbpclaw-docs) 中浏览源码。

| 章节 | 主题 |
|------|------|
| [快速开始](https://docs.vbpclaw.sh/#what-is-vbpclaw) | 安装、快速启动、配置、Web 控制台导览 |
| [核心概念](https://docs.vbpclaw.sh/#how-vbpclaw-works) | 智能体循环、会话、工具、记忆、多租户 |
| [智能体](https://docs.vbpclaw.sh/#creating-agents) | 创建智能体、上下文文件、人格设定、共享与访问 |
| [提供商](https://docs.vbpclaw.sh/#providers-overview) | Anthropic、OpenAI、OpenRouter、Gemini、DeepSeek，以及 15+ 个其他提供商 |
| [渠道](https://docs.vbpclaw.sh/#channels-overview) | Telegram、Discord、Slack、飞书、Zalo、WhatsApp、WebSocket |
| [智能体团队](https://docs.vbpclaw.sh/#teams-what-are-teams) | 团队、任务板、消息传递、委托与交接 |
| [高级功能](https://docs.vbpclaw.sh/#custom-tools) | 自定义工具、MCP、技能、定时任务、沙箱、钩子、RBAC |
| [部署](https://docs.vbpclaw.sh/#deploy-docker-compose) | Docker Compose、数据库、安全、可观测性、Tailscale |
| [参考](https://docs.vbpclaw.sh/#cli-commands) | CLI 命令、REST API、WebSocket 协议、环境变量 |
...
VBPClaw 基于原始的 [OpenClaw](https://github.com/openclaw/openclaw) 项目构建。我们对启发这个 Go 移植版本的架构设计和愿景深表感谢。
