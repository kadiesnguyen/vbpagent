...
3. VBPClaw renders the command template with shell-escaped arguments, checks deny patterns, and executes with timeout
...
Connect external [Model Context Protocol](https://modelcontextprotocol.io) servers to extend agent capabilities. MCP tools are registered transparently into VBPClaw's tool registry and invoked like any built-in tool.
