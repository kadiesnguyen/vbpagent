#!/usr/bin/env bash
# VBPClaw Docker setup — generates .env and docker-compose command for your chosen variant.
#
# Usage:
...
DEV_MODE=false
WITH_UI=false
PORT="${VBPCLAW_PORT:-18790}"
UI_PORT="${VBPCLAW_UI_PORT:-3000}"
PG_PORT="${POSTGRES_PORT:-5432}"
DATA_DIR="${VBPCLAW_DATA_DIR:-./vbpclaw-data}"
...
if [ -z "$VARIANT" ]; then
  echo "Select VBPClaw Docker variant:"
  echo ""
...
echo ""
echo "Setting up VBPClaw (${VARIANT})..."
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  cat > "$ENV_FILE" <<EOF
# VBPClaw environment — generated by setup-docker.sh
# Variant: ${VARIANT}

# Gateway
VBPCLAW_GATEWAY_TOKEN=${GATEWAY_TOKEN}
VBPCLAW_ENCRYPTION_KEY=${ENCRYPTION_KEY}

# Ports
VBPCLAW_PORT=${PORT}
VBPCLAW_UI_PORT=${UI_PORT}
POSTGRES_PORT=${PG_PORT}

# PostgreSQL
POSTGRES_USER=vbpclaw
...
# VBPClaw Docker Compose — variant: ${VARIANT}
...
services:
  postgres:
    ...
    environment:
      POSTGRES_USER: \${POSTGRES_USER:-vbpclaw}
      POSTGRES_PASSWORD: \${POSTGRES_PASSWORD:-vbpclaw}
      POSTGRES_DB: \${POSTGRES_DB:-vbpclaw}
    volumes:
      - postgres-data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U \${POSTGRES_USER:-vbpclaw}"]
      interval: 5s
      timeout: 5s
      retries: 10
...
cat >> "$COMPOSE_FILE" <<YAML
    ports:
      - "\${VBPCLAW_PORT:-18790}:18790"
    env_file:
      - .env
    environment:
      - VBPCLAW_HOST=0.0.0.0
      - VBPCLAW_PORT=18790
      - VBPCLAW_CONFIG=/app/data/config.json
      - VBPCLAW_SKILLS_DIR=/app/data/skills
      - VBPCLAW_WORKSPACE=/app/workspace
      - VBPCLAW_POSTGRES_DSN=postgres://\${POSTGRES_USER:-vbpclaw}:\${POSTGRES_PASSWORD:-vbpclaw}@postgres:5432/\${POSTGRES_DB:-vbpclaw}?sslmode=disable
    volumes:
      # Persistent data: config, agent files, runtime packages (pip/npm)
      - vbpclaw-data:/app/data
      # Workspace: agent working directory for file operations
      - vbpclaw-workspace:/app/workspace
      # Custom skills: user-installed skills persist across restarts
      - vbpclaw-skills:/app/skills
      # Storage: uploaded media, documents
      - vbpclaw-storage:/app/storage
    depends_on:
      postgres:
...
echo "── Volumes ──"
echo ""
echo "  vbpclaw-data      → /app/data       Config, runtime packages (pip/npm cache)"
echo "  vbpclaw-workspace → /app/workspace  Agent file operations"
echo "  vbpclaw-skills    → /app/skills     Custom installed skills"
echo "  vbpclaw-storage   → /app/storage    Uploaded media & documents"
echo "  postgres-data    → PostgreSQL data"
echo ""
echo "── Useful commands ──"
echo ""
echo "  docker compose logs -f vbpclaw      # View logs"
echo "  docker compose exec vbpclaw vbpclaw version   # Check version"
