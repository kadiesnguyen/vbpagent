...
func (m *ContactsMethods) Register(router *gateway.MethodRouter) {
	router.Register(vbpclawprotocol.MethodZaloPersonalContacts, m.handleContacts)
}

func (m *ContactsMethods) handleContacts(ctx context.Context, client *gateway.Client, req *vbpclawprotocol.RequestFrame) {
	var params struct {
	...
	if len(inst.Credentials) == 0 {
		client.SendResponse(vbpclawprotocol.NewErrorResponse(req.ID, vbpclawprotocol.ErrInvalidRequest, "instance has no credentials — complete QR login first"))
		return
	}

	// Prevent concurrent fetches for same instance
	if _, loaded := m.activeFetches.LoadOrStore(params.InstanceID, struct{}{}); loaded {
		client.SendResponse(vbpclawprotocol.NewErrorResponse(req.ID, vbpclawprotocol.ErrInvalidRequest, "contacts fetch already in progress"))
		return
	}
	...
	if err := json.Unmarshal(inst.Credentials, &creds); err != nil {
		client.SendResponse(vbpclawprotocol.NewErrorResponse(req.ID, vbpclawprotocol.ErrInternal, "failed to parse credentials"))
		return
	}
	...
	if err := protocol.LoginWithCredentials(fetchCtx, sess, *protoCred); err != nil {
		...
		client.SendResponse(vbpclawprotocol.NewErrorResponse(req.ID, vbpclawprotocol.ErrInternal, "Zalo login failed — credentials may be expired, try QR login again"))
		return
	}
	...
	if err := g.Wait(); err != nil {
		...
		client.SendResponse(vbpclawprotocol.NewErrorResponse(req.ID, vbpclawprotocol.ErrInternal, "failed to fetch contacts"))
		return
	}

	client.SendResponse(vbpclawprotocol.NewOKResponse(req.ID, map[string]any{
		"friends": friends,
		"groups":  groups,
	}))
}
