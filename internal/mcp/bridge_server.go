...
// BridgeToolNames is the subset of VBPClaw tools exposed via the MCP bridge.
...
// NewBridgeServer creates a StreamableHTTPServer that exposes VBPClaw tools as MCP tools.
...
func NewBridgeServer(reg *tools.Registry, version string, msgBus *bus.MessageBus) *mcpserver.StreamableHTTPServer {
	srv := mcpserver.NewMCPServer("vbpclaw-bridge", version,
		mcpserver.WithToolCapabilities(false),
	)

	// Register each safe tool from the VBPClaw registry
	var registered int
...
// convertToMCPTool converts a VBPClaw tools.Tool into an mcp-go Tool.
...
// makeToolHandler creates a ToolHandlerFunc that delegates to the VBPClaw tool registry.
...
func makeToolHandler(reg *tools.Registry, toolName string, msgBus *bus.MessageBus) mcpserver.ToolHandlerFunc {
	return func(ctx context.Context, req mcpgo.CallToolRequest) (*mcpgo.CallToolResult, error) {
		...
		// VBPClaw's agent loop never sees result.Media from bridge tool calls.
