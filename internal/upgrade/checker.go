...
func FormatError(s *SchemaStatus) string {
	if s.Dirty {
		return fmt.Sprintf(
			"Database schema is in a dirty state (version %d).\n"+
				...
				"  Fix:  ./vbpclaw migrate force %d\n"+
				"  Then: ./vbpclaw upgrade\n",
	...
	if s.CurrentVersion > s.RequiredVersion {
		return fmt.Sprintf(
			"Database schema (v%d) is newer than this binary (requires v%d).\n"+
				"You may be running an older version of vbpclaw.\n\n"+
				"  Fix: upgrade your vbpclaw binary to the latest version.\n",
	...
	return fmt.Sprintf(
		"Database schema is outdated: current v%d, required v%d.\n\n"+
			"  Run:  ./vbpclaw upgrade\n"+
			"  Or:   ./vbpclaw migrate up   (SQL-only, no data hooks)\n\n"+
			"  Docker/CI: set VBPCLAW_AUTO_UPGRADE=true to upgrade automatically on startup.\n",
		s.CurrentVersion, s.RequiredVersion,
	)
}
