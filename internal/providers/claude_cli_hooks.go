...
// that enforce VBPClaw's security policies (shell deny patterns, path restrictions).
...
func BuildCLIHooksConfig(workspace string, restrictToWorkspace bool) (string, func(), error) {
	tmpDir := filepath.Join(os.TempDir(), "vbpclaw-cli-hooks")
...
// generateHookScript creates a bash script that enforces VBPClaw security policies.
...
# VBPClaw security hook for Claude CLI PreToolUse.
