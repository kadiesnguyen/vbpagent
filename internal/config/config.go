...
// Config is the root configuration for the VBPClaw Gateway.
type Config struct {
	DataDir   string          `json:"data_dir,omitempty"` // persistent data directory (default: ~/.vbpclaw/data)
...
type TailscaleConfig struct {
	Hostname  string `json:"hostname"`             // Tailscale machine name (e.g. "vbpclaw-gateway")
	StateDir  string `json:"state_dir,omitempty"`  // persistent state directory (default: os.UserConfigDir/tsnet-vbpclaw)
	AuthKey   string `json:"-"`                    // from env VBPCLAW_TSNET_AUTH_KEY only
...
type DatabaseConfig struct {
	PostgresDSN    string `json:"-"` // from env VBPCLAW_POSTGRES_DSN only
	RedisDSN       string `json:"-"` // from env VBPCLAW_REDIS_DSN only (optional, requires -tags redis)
	StorageBackend string `json:"-"` // from env VBPCLAW_STORAGE_BACKEND only ("postgres" or "sqlite", default "postgres")
	SQLitePath     string `json:"-"` // from env VBPCLAW_SQLITE_PATH only (default: {dataDir}/vbpclaw.db)
}
...
type SandboxConfig struct {
	...
	Image           string            `json:"image,omitempty"`            // Docker image (default: "vbpclaw-sandbox:bookworm-slim")
...
type TelemetryConfig struct {
	...
	ServiceName  string                     `json:"service_name,omitempty"`  // OTEL service name (default "vbpclaw-gateway")
