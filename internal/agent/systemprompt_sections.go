...
func buildGoogleWorkspaceSection(emails []string) []string {
	...
	lines = append(lines,
		...
		"",
		"IMPORTANT: After creating or modifying any Google Drive file (Docs, Sheets, Slides, etc.),",
		"always share the direct link to the file with the user. The link format is:",
		"https://docs.google.com/spreadsheets/d/{fileId} (for Sheets),",
		"https://docs.google.com/document/d/{fileId} (for Docs),",
		"https://docs.google.com/presentation/d/{fileId} (for Slides),",
		"https://drive.google.com/file/d/{fileId} (for other files).",
		"Extract the fileId from the tool response and always provide the full clickable link.",
		"",
		"IMPORTANT: When writing data to a Google Sheet, never assume the sheet tab name is 'Sheet1'.",
		"The default tab name depends on locale (e.g. 'Trang tính 1' in Vietnamese, 'Sheet1' in English).",
		"Always call get_spreadsheet first to retrieve the actual sheet tab name, then use that name",
		"in the range parameter (e.g. 'Trang tính 1!A1' instead of 'Sheet1!A1').",
		"Alternatively, omit the sheet name prefix entirely and use just the cell range (e.g. 'A1:B10'),",
		"which writes to the first sheet regardless of its name.",
		"",
		"To rename a Google Drive file: use manage_drive with operation=rename, fileId=<id>, name=<newName>.",
		"To rename a sheet tab: use manage_sheets with operation=renameSheet, spreadsheetId=<id>,",
		"and jsonBody='{\"requests\":[{\"updateSheetProperties\":{\"properties\":{\"sheetId\":0,\"title\":\"<tabName>\"},\"fields\":\"title\"}}]}'",
		"(use sheetId=0 for the first tab; call get first to find sheetId for other tabs).",
		"Always rename the file and tab immediately after creating them, before writing data.",
		"",
	)
	return lines
}
