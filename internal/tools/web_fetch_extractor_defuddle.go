...
const (
	defuddleBaseURL = "https://fetch.vbpclaw.sh/"
	defuddleTimeout = 10 * time.Second
	defuddleMaxBody = 1 << 20 // 1MB
)

// DefuddleExtractor calls the fetch.vbpclaw.sh Cloudflare Worker to extract
...
// Extract sends a GET request to fetch.vbpclaw.sh/<domain>/<path> (no scheme)
